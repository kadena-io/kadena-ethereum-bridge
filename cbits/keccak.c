...
/* Implementations of exported function */

ETH_KECCAK256_CTX *keccak256_newctx()
{
    ETH_KECCAK256_CTX * ctx = ((OSSL_FUNC_digest_newctx_fn *) dispatch256(OSSL_FUNC_DIGEST_NEWCTX))(NULL);
...
ETH_KECCAK512_CTX *keccak512_newctx()
{
    ETH_KECCAK512_CTX * ctx = ((OSSL_FUNC_digest_newctx_fn *) dispatch512(OSSL_FUNC_DIGEST_NEWCTX))(NULL);
...
int keccak512_init(ETH_KECCAK256_CTX *ctx) {
    return ((OSSL_FUNC_digest_init_fn *) dispatch512(OSSL_FUNC_DIGEST_INIT))(ctx, NULL);
}

int keccak256_reset(ETH_KECCAK256_CTX *ctx) {
    return keccak256_init(ctx);
}

int keccak512_reset(ETH_KECCAK512_CTX *ctx) {
    return keccak512_init(ctx);
}

int keccak256_update(ETH_KECCAK256_CTX *ctx, const void *p, size_t l)
{
    return ((OSSL_FUNC_digest_update_fn *) dispatch256(OSSL_FUNC_DIGEST_UPDATE))(ctx, p, l);
}

int keccak512_update(ETH_KECCAK512_CTX *ctx, const void *p, size_t l)
{
    return ((OSSL_FUNC_digest_update_fn *) dispatch512(OSSL_FUNC_DIGEST_UPDATE))(ctx, p, l);
}

int keccak256_final(ETH_KECCAK256_CTX *ctx, unsigned char *md)
{
    size_t l;
    return ((OSSL_FUNC_digest_final_fn *) dispatch256(OSSL_FUNC_DIGEST_FINAL))(ctx, md, &l, 32);
}

int keccak512_final(ETH_KECCAK512_CTX *ctx, unsigned char *md)
{
    size_t l;
    return ((OSSL_FUNC_digest_final_fn *) dispatch512(OSSL_FUNC_DIGEST_FINAL))(ctx, md, &l, 64);
}

void keccak256_freectx(ETH_KECCAK256_CTX *ctx)
{
    return ((OSSL_FUNC_digest_freectx_fn *) dispatch256(OSSL_FUNC_DIGEST_FREECTX))(ctx);
}

void keccak512_freectx(ETH_KECCAK512_CTX *ctx)
{
